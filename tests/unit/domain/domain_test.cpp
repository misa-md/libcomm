...
TEST(domain_test_decomposition, domain_test) {
  ...
  int local_grid[3] = {(_domain->sub_box_lattice_size[0]), (_domain->sub_box_lattice_size[1]),
...
TEST(bcc_domain_local_lattice_coord, bcc_domain_test) {
  ...
  EXPECT_EQ(_domain->dbx_sub_box_lattice_size[0],
            _domain->dbx_local_sub_box_lattice_region.x_high - _domain->dbx_local_sub_box_lattice_region.x_low);
  EXPECT_EQ(_domain->dbx_sub_box_lattice_size[1],
            _domain->dbx_local_sub_box_lattice_region.y_high - _domain->dbx_local_sub_box_lattice_region.y_low);
  EXPECT_EQ(_domain->dbx_sub_box_lattice_size[2],
            _domain->dbx_local_sub_box_lattice_region.z_high - _domain->dbx_local_sub_box_lattice_region.z_low);

  EXPECT_EQ(_domain->dbx_ghost_extended_lattice_size[0],
            _domain->dbx_local_ghost_ext_lattice_region.x_high - _domain->dbx_local_ghost_ext_lattice_region.x_low);
  EXPECT_EQ(_domain->dbx_ghost_extended_lattice_size[1],
            _domain->dbx_local_ghost_ext_lattice_region.y_high - _domain->dbx_local_ghost_ext_lattice_region.y_low);
  EXPECT_EQ(_domain->dbx_ghost_extended_lattice_size[2],
            _domain->dbx_local_ghost_ext_lattice_region.z_high - _domain->dbx_local_ghost_ext_lattice_region.z_low);
}
...
TEST(domain_set_ghost_size, domain_test) {
  ...
  double cutoff_radius_factor = 1.1421;
  comm::Domain *_domain =
      getDomainInstance(space, static_cast<int>(ceil(cutoff_radius_factor) + 1), lattice_const, cutoff_radius_factor);
...
TEST(domain_rescale_box_test, domain_test) {
  ...
  EXPECT_DOUBLE_EQ(p_domain->meas_global_length[2], scale_factor * space[2] * old_lattice_const);
  ...
  EXPECT_DOUBLE_EQ(p_domain->meas_global_region.high[0], scale_factor * space[0] * old_lattice_const);
  EXPECT_DOUBLE_EQ(p_domain->meas_global_region.high[1], scale_factor * space[1] * old_lattice_const);
  EXPECT_DOUBLE_EQ(p_domain->meas_global_region.high[2], scale_factor * space[2] * old_lattice_const);

  EXPECT_DOUBLE_EQ(p_domain->meas_sub_box_region.low[0], grid_coord[0] * 50 * old_lattice_const * scale_factor);
  EXPECT_DOUBLE_EQ(p_domain->meas_sub_box_region.low[1], grid_coord[1] * 60 * old_lattice_const * scale_factor);
  EXPECT_DOUBLE_EQ(p_domain->meas_sub_box_region.low[2], grid_coord[2] * 72 * old_lattice_const * scale_factor);

  EXPECT_DOUBLE_EQ(p_domain->meas_sub_box_region.high[0], (grid_coord[0] + 1) * 50 * old_lattice_const * scale_factor);
