...
TEST(sector_fw_region_area_test, sector_fw_region_test) {
  ...
  const int64_t space[3] = {local_space[0] * grid_size[0], local_space[1] * grid_size[1],
  ...
  for (int s = 0; s < 8; s++) {
    for (int dim = 0; dim < 3; dim++) {
      ...
      if (local_space[0] % 2 == 0 && local_space[1] % 2 == 0 && local_space[2] % 2 == 0) {
        if (dim == comm::DIM_X) {
          EXPECT_EQ(region_vol_total,
                    ghost_size * (local_space[1] / 2 + 2 * ghost_size) * (local_space[2] / 2 + 2 * ghost_size));
        } else if (dim == comm::DIM_Y) {
          EXPECT_EQ(region_vol_total,
                    ghost_size * (local_space[0] / 2 + 2 * ghost_size) * (local_space[2] / 2 + 2 * ghost_size));
        } else {
          EXPECT_EQ(region_vol_total,
                    ghost_size * (local_space[0] / 2 + 2 * ghost_size) * (local_space[1] / 2 + 2 * ghost_size));
        }
      }
    }
  }
